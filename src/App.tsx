    ...
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel p-6 flex flex-col h-full overflow-y-auto max-h-[90vh]">
      ...
      <div className="space-y-4 mb-6">
        <div className="bg-black/30 p-4 rounded-xl border border-gray-800">
          <div className="flex justify-between items-center mb-2"><label className="text-sm font-bold text-white">ЦНС (Рівень стресу)</label><span className="text-red-400 font-bold text-lg">{scaleCns}/10</span></div>
          <input type="range" min="1" max="10" value={scaleCns} onChange={e => { setScaleCns(parseInt(e.target.value)); triggerHaptic(); }} className="w-full accent-red-500" />
        </div>

        <div className="bg-black/30 p-4 rounded-xl border border-gray-800">
          <div className="flex justify-between items-center mb-2"><label className="text-sm font-bold text-white">Рівень енергії</label><span className="text-yellow-400 font-bold text-lg">{scaleEnergy}/10</span></div>
          <input type="range" min="1" max="10" value={scaleEnergy} onChange={e => { setScaleEnergy(parseInt(e.target.value)); triggerHaptic(); }} className="w-full accent-yellow-400" />
        </div>

        <div className="bg-black/30 p-4 rounded-xl border border-gray-800">
          <div className="flex justify-between items-center mb-2"><label className="text-sm font-bold text-white">Ментальний фокус</label><span className="text-blue-400 font-bold text-lg">{scaleMental}/10</span></div>
          <input type="range" min="1" max="10" value={scaleMental} onChange={e => { setScaleMental(parseInt(e.target.value)); triggerHaptic(); }} className="w-full accent-blue-400" />
        </div>
      </div>
...
function ResultScreen({ recipe, weatherTemp, weatherCond, onDone }: { recipe: Recipe, weatherTemp: number, weatherCond: string, onDone: () => void }) {
  return (
    <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="glass-panel flex flex-col h-full overflow-y-auto max-h-[95vh] p-0 border-primary/30">
      <div className="relative h-48 bg-gradient-to-br from-gray-900 to-black border-b border-primary/20 flex flex-col items-center justify-end pb-4 overflow-hidden">
        <div className="absolute inset-0 opacity-40 bg-cover bg-center mix-blend-screen" style={{ backgroundImage: `url(${recipe.avatar_image})` }}></div>
        <div className="relative z-10 text-center px-4 bg-black/60 backdrop-blur-sm p-3 rounded-xl border border-white/10 mt-auto">
          <h2 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-400 uppercase tracking-widest drop-shadow-[0_0_10px_rgba(0,255,204,0.8)]">{recipe.avatar_name}</h2>
          <p className="text-[10px] text-gray-300 mt-1 italic leading-tight">"{recipe.avatar_slogan}"</p>
        </div>
      </div>
